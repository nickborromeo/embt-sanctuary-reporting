class LicensesController < ApplicationController
  ...
  #TODO Fix the query to remove cartesean product result
  
  def get_licenses
    date1 = params[:start_date] || ""
    date2 = params[:end_date] || ""
    
    @licenses = ActiveRecord::Base.connection.select_rows("Select CONCAT(C.First_Name, CONCAT(' ', C.Last_Name)),  ssb.serial_number, C.E_Mail, con.COMPANY, slp.TITLE, sp.DESCRIPTION, C.Country, sli.created from WEBMGR.TABLE_CONTACT c, WEBMGR.SANCT_SERIALNO_BANK ssb, WEBMGR.SANCT_LICENSE_ISSUANCE sli, WEBMGR.SANCT_LICENSE_PACKAGE slp,  SANCTUARY.PART_INSTANCE_SERIAL pis, SANCTUARY.ORDER_ITEM oi, SANCTUARY.ORDER_CONTACT oc, SANCTUARY.CONTACT con, WEBMGR.SANCT_PRODUCT sp, WEBMGR.SANCT_PRODUCT_VERSION spv, WEBMGR.SANCT_PRODUCT_SKU spsku, WEBMGR.SANCT_LICENSE_CONFIG slc where (c.USERID = sli.USER_ID) and (ssb.SERIAL_ID = sli.SERIAL_ID) and (ssb.SERIAL_ID = pis.SERIAL_ID) and (slp.PACKAGE_ID = ssb.PACKAGE_ID) and (pis.INSTANCE_ID = oi.INSTANCE_ID) and (oi.ORDER_ID = oc.ORDER_ID) And (Oc.Contact_Id = Con.Contact_Id) AND (sp.PRODUCT_ID = spv.PRODUCT_ID) AND (spv.VERSION_ID = spsku.VERSION_ID) AND (spsku.VERSION_ID  = slc.VERSION_ID) AND (slc.CONFIG_ID = slp.HOST_LICENSE_CFG) AND (c.upper_company_name IS NULL OR c.upper_company_name not like 'EMBARCADERO%') AND c.upper_e_mail not like '%@CODEGEAR.COM'And C.Upper_E_Mail Not Like '%@EMBARCADERO.COM'AND con.upper_company not like 'EMBARCADERO%' AND con.upper_email not like '%@CODEGEAR.COM'AND con.upper_email not like '%@EMBARCADERO.COM'and c.employee = 'F' and con.employee = 'F' and slp.company_id = 1 And Slp.Division_Id = 2 And Sli.Created Between To_Date ('"+date1+"', 'mm/dd/yyyy') And To_Date ('"+date2+"', 'mm/dd/yyyy') ORDER BY sli.created DESC")
